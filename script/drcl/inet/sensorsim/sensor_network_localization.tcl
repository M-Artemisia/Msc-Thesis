# Tcl file for localization algorithm in sensor networks.
# Author: Hyuk Lim 
# Date: 8/23/2004

#
#Modified by Nicholas Merizzi to reflect changes 
#to the original design as discussed at 
#	http://www.cas.mcmaster.ca/~merizzn/



source "E:/University/Msc/thesis/simulator/JSim/jsim-1.3-project/script/test/include.tcl"
...
for {set i [expr $sink_id + 1]} {$i < [expr $node_num - $target_node_num]} {incr i} {
	...
	# mkdir drcl.inet.sensorsim.SensorApp app
	# use SensorLocApp
	mkdir drcl.inet.sensorsim.APS.SensorLocApp app
	#
	...
	# Hyuk: anchor nodes need to know its location
...
for {set i [expr $sink_id + 1]} {$i < [expr $node_num - $target_node_num]} {incr i} {
	connect -c n$i/app/.locerr@ -to $plot_/$i@0

	if { $testflag } {
		attach -c $testfile/in@ -to n$i/app/.locerr@
	}
}
...
# sensors
set dist 60
! n1/mobility setPosition 0.0 [expr 100.0+$dist] 250.0 0.0
! n2/mobility setPosition 0.0 [expr 100.0+2*$dist] 250.0 0.0
! n3/mobility setPosition 0.0 [expr 100.0+3*$dist] 250.0 0.0
! n4/mobility setPosition 0.0 [expr 100.0+4*$dist] 250.0 0.0
! n5/mobility setPosition 0.0 [expr 100.0+5*$dist] 250.0 0.0

! n1/app setAnchor true
! n5/app setAnchor true
