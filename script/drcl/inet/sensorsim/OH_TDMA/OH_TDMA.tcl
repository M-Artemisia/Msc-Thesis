...
source "H:/prj sim/LA/jsim-1.3-project/script/test/include.tcl"
#source "../../../../test/include.tcl"

cd [mkdir -q drcl.comp.Component /oneHopTDMA]
...
for {set i 0} {$i < [expr $sink_id + 1]} {incr i} {
	...
	cd n$i
	mkdir drcl.inet.sensorsim.OneHopTDMA.SinkAppTDMA app
	! app setNid $i
	...
	connect mobility/.report@ -and /oneHopTDMA/tracker/.node@
	connect wphy/down@ -to /oneHopTDMA/channel/.node@

	! /oneHopTDMA/channel attachPort $i [! wphy getPort .channel]
...
for {set i [expr $sink_id + 1]} {$i < [expr $node_num - $target_node_num]} {incr i} {
	...
	#NICHOLAS*****
	mkdir drcl.inet.sensorsim.OneHopTDMA.OneHopAppTDMA app
	! app setNid $i
	! app setSinkNid $sink_id
	! app setNn_ [expr $node_num - 1]
	! app setIs_uAMPS 1
	...
	! /oneHopTDMA/chan attachPort $i [! phy getPort .channel]

	# connect the nodes to the propagation model
	connect phy/.propagation@ -and /oneHopTDMA/seismic_Prop/.query@
	...
	#*************
	#NICHOLAS
	! wphy setMIT_uAMPS 1	   ; #to maintain power awareness
	! wphy setOneHopMode 1	   ; #ignores incomming packets
	! wphy setOneHopModeTDMA 1 ; #allows for card to go to sleep after Tx
	connect wphy/.channelCheck@ -and mac/.wphyRadioMode@
	connect app/.energy@ -and wphy/.appEnergy@
	#*************
...
if { $target_node_num == 0 } {
	...
	for {set i [expr $node_num - $target_node_num]} {$i < $node_num} {incr i} {
		...
		connect phy/down@ -to /oneHopTDMA/chan/.node@
...
for {set i [expr $sink_id + 1]} {$i < $node_num} {incr i} {
	...
	connect n$i/mobility/.report_sensor@ -and /oneHopTDMA/nodetracker/.node@
...
for {set i [expr $sink_id + 1]} {$i < [expr $node_num - $target_node_num]} {incr i} {
   ! n$i/mobility setPosition 0.0 [expr rand()*5] [expr rand() * 5] 0.0
}
...
proc wsnLoop { } { 
	...
	#perform a send if they are not all dead
