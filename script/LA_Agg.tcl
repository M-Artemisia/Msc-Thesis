                             ...
                             set darC 1
                     #********************************************
                             cd [mkdir -q drcl.comp.Component /LA_Agg]
                             ...
                             cd n$i
                             mkdir drcl.inet.sensorsim.LA_Agg.LA_SinkApp app
                             ! app setNid $i
                             ...
                             connect mobility/.report@ -and /LA_Agg/tracker/.node@
                             connect wphy/down@ -to /LA_Agg/channel/.node@

                             !  /LA_Agg/channel attachPort $i [! wphy getPort .channel]
                             # connect wphy/.channelCheck@ -and mac/.wphyRadioMode@
                             ...
                             mkdir drcl.inet.sensorsim.LA_Agg.LA_SourceApp app
                             ! app setNid $i
                             ! app setSinkNid $sink_id
                             ! app setCoherentThreshold 1000.0
                         ...
                         ! /LA_Agg/chan attachPort $i [! phy getPort .channel]

                         # connect the nodes to the propagation model
                         connect phy/.propagation@ -and /LA_Agg/seismic_Prop/.query@
                         ...
                         if { $target_node_num == 0 } {
                             ...
                             for {set i [expr $node_num - $target_node_num]} {$i < $node_num} {incr i} {
                                 ...
                                 # connect phy layers to sensor channel so that nodes can send
                                 connect phy/down@ -to /LA_Agg/chan/.node@
                         ...
                         # connect the mobility model of each node to the node position tracker
                             connect n$i/mobility/.report_sensor@ -and /LA_Agg/nodetracker/.node@
                         ...
                         proc wsnLoop { } {
                             ...
                             for {set i [expr $sink_id + 1]} {$i < [expr $node_num - $target_node_num]} {incr i} {
                               ...
                               set filenameUe "MY_RESULT/LA/UsedEnergyLA.txt"
                                set filenameT "MY_RESULT/LA/TotalInPktsLA.txt"
                                set filenameV "MY_RESULT/LA/VirtualPktsLA.txt"
                               ...
                               if { $dead_sensors != 0 } {
                                ...
                                set filenameE "MY_RESULT/LA/EnrgyLA.txt"
