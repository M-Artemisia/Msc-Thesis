        ...
        set darC 2
#********************************************
        cd [mkdir -q drcl.comp.Component /esna_LAg]
        ...
        cd n$i
        mkdir drcl.inet.sensorsim.esna_LAg.esnaLA_SinkApp app
        ! app setNid $i
        ...
        connect mobility/.report@ -and /esna_LAg/tracker/.node@
        connect wphy/down@ -to /esna_LAg/channel/.node@

        !  /esna_LAg/channel attachPort $i [! wphy getPort .channel]
        # connect wphy/.channelCheck@ -and mac/.wphyRadioMode@
        ...
        mkdir drcl.inet.sensorsim.esna_LAg.esnaLA_SourceApp app
        ! app setNid $i
        ! app setSinkNid $sink_id
        ! app setCoherentThreshold 1000.0
    ...
    ! /esna_LAg/chan attachPort $i [! phy getPort .channel]

    # connect the nodes to the propagation model
    connect phy/.propagation@ -and /esna_LAg/seismic_Prop/.query@
    ...
    if { $target_node_num == 0 } {
        ...
        for {set i [expr $node_num - $target_node_num]} {$i < $node_num} {incr i} {
            ...
            # connect phy layers to sensor channel so that nodes can send
            connect phy/down@ -to /esna_LAg/chan/.node@
    ...
    # connect the mobility model of each node to the node position tracker
        connect n$i/mobility/.report_sensor@ -and /esna_LAg/nodetracker/.node@
    ...
    proc wsnLoop { } {
        ...
        for {set i [expr $sink_id + 1]} {$i < [expr $node_num - $target_node_num]} {incr i} {
          ...
          set filenameUe "MY_RESULT/esna/UsedEnergyesna.txt"
           set filenameT "MY_RESULT/esna/TotalInPktsesna.txt"
           set filenameV "MY_RESULT/esna/VirtualPktsesna.txt"
          ...
          if { $dead_sensors != 0 } {
           ...
           set filenameE "MY_RESULT/esna/Enrgyesna.txt"
