        ...
        */
        if ( rx_state_ == MAC_IDLE ) {
            ...
            */
            ...
            //Added by Esnaashari->Starts
            //If the new packet is sent with enough power to swamp out the reception of pktRx_, pktRx_ is dropped
            // and this new packet is received
            else if (msg.getRxPr() / txinfo_pktRx_RxPr >= txinfo_pktRx_CPThresh) {
                //This silly J-Sim! We have to add the dropped packet (pktRx_) to dropped_packets vector for J-Sim
                // statistics. Make sure that each packet has a different event id
                boolean alreadyDropped = false;
                long src_id = pktRx_.getSa();
                long dst_id = pktRx_.getDa();
                int event_id =  ((SensorPacket)(((InetPacket)((LLPacket)pktRx_.getBody()).getBody()).getBody())).getEventID();
                ...
                //pktRx_ is added to the dropped_packets vector
                SET_RX_STATE(MAC_RECV);
                pktRx_ = (Mac_Sensor_Packet) (msg.getPkt());
                //By the way, recevier must be busy until the end of the receive time of both packets 
                if (TX_Time(pktRx_) > rx_timer_.expire()) {
                    rx_timer_.stop();
                    rx_timer_.start(TX_Time(pktRx_));
                }
                txinfo_pktRx_RxPr = msg.getRxPr();
                txinfo_pktRx_CPThresh = msg.getCPThresh();
                //Since this new packet can be captured, it is not erronous
                pktRx_.setForcedError(false);
            }
            //Added by Esnaashari->Ends
            else {
