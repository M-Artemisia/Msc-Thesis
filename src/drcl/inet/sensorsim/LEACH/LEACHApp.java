...
public class LEACHApp  extends SensorApp implements drcl.comp.ActiveComponent
{
    ...
    Added By M.Asemani
     */
    environment env=new environment(5.0,5.0,0.0,1.0,getTime());
    ...
    */
    public void GoToSleep()
    {
        ...
        if (this.cpuMode != 0) {
            this.setCPUMode(0);    //CPU_IDLE=0, CPU_SLEEP=1, CPU_ACTIVE=2, CPU_OFF=3
        }
    ...
    */
    protected synchronized void recvDATA(LEACH_Data_Packet msg)
    {
        ...
        added By M.Asemani
         */
        double newData = ((Double)msg.getBody()).doubleValue();
        int Done =0;
        for(int i=0; i< CHData.size();i++){
            double d=((SenseData)CHData.elementAt(i)).data;
            int    agcnt=((SenseData)CHData.elementAt(i)).AggCount;
            if(Math.abs(d-newData) < EPSILON){
                ((SenseData)CHData.elementAt(i)).data=(d*agcnt+newData) /(agcnt+1);
                ((SenseData)CHData.elementAt(i)).AggCount++;
                Done =1;
                break;
           }
        }
        if(Done == 0){
            CHData.add(new SenseData(newData,1));
        }
    ...
    */
    protected synchronized void recvSensorEvent(Object data_)
    {
        ...
        phenomenon = new Double(env.getWarmth(loc,getTime()));
        return;
    }
  protected void SenseDataFromEnv(){
        double [] loc = new double[3];   loc[0] = getX() ;         loc[1] = getY();         loc[2] = getZ();
        phenomenon = new Double(env.getWarmth(loc,getTime()));
      setTimeout("SenseDataFromEnv",100 + generator.nextDouble()*5 );
        return;
   }
    /**
    ...
    */
    protected void SendDataToBS() {
     ...
     //   int datasize = 37;
        ...
        for(int i=0;i<CHData.size();i++){
            ...
            //create the packet to be sent out
            LEACH_Data_Packet newPacket = new LEACH_Data_Packet(this.nid, this.currentCH,this.CHData.elementAt(i),getTime(),bsCode,207);
            ...
            if (!(this.sensorDEAD)) {
                //                                                  (int UniBcast_flag_, long dst_, long src_, int size_, int type_, int eventID_, long target_nid_, Object body_)
                downPort.doSending(new SensorAppWirelessAgentContract.Message(SensorAppWirelessAgentContract.UNICAST_SENSOR_PACKET, this.sink_nid, this.nid, this.sinkPos, 207,LEACH_DATA, eID, this.nid, newPacket)) ;
                eID = eID + 1;          //every message sent is associated with an event ID
    ...
    protected void SendMyDataToBS()
    {
       ...
       // this.CHData.add(phenomenon);
        SenseData data=new SenseData(phenomenon.doubleValue(),1);

        //create the packet to be sent out
        LEACH_Data_Packet newPacket = new LEACH_Data_Packet(this.nid, this.currentCH,data , getTime(),bsCode, datasize);
