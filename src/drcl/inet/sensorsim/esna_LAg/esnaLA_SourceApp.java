...
*/
public class esnaLA_SourceApp extends SensorApp implements ActiveComponent {
...
protected void _stop(){
   if (rTimer != null)
      ...
      File DieFile;
   DieFile = new File("C:/simRslt/esnaLaAgg/die.txt");
         try{
...
//===============================================================================
    ...
    if(data instanceof Double ){
     ...
     if(Math.abs(pktdata.doubleValue()-SensedData.doubleValue()) < EPSILON ){
        ...
        SendAckPacket(pckt.id,dar.doubleValue(),DstLoc);        
   }
 }
 //===============================================================================
...
//===============================================================================
   protected void SendAckPacket(long Dst_id,/*30 dei*/double dar,double [] DstLoc){
    this.checkAlive();
    double [] loc = new double[3];    loc[0] = getX() ;  loc[1] = getY();         loc[2] = getZ();
    LA_ControlPacket pckt=new LA_ControlPacket(nid,"Ack Data Packet",8,loc,RemainEnergy(),dar);
...
protected Double CalculateDAR(double pktdata){
     double x = (Math.min(pktdata+EPSILON,SensedData.doubleValue()+EPSILON)- Math.max(pktdata-EPSILON,SensedData.doubleValue()-EPSILON))/2*EPSILON;
    if(x>= 0 )
        return new Double(x);
    else
...
public    void createLA(){
    ...
    la= new LA(m_Neighbours.size(),m_Neighbours,REWARD_PARAM,PENALTY_PARAM,1,1,1);
    }
   //=============================================================================== 
    public    void DecideRewORPen(int i){
        ...
        if(la == null){
            ...
            la.rewardOrPenalize(i,new Double(n.getDAR()));
        }
